...
class ComponentTypeLoader(
    ...
    override fun load() {
        ...
        val groups = archive.keys
        groups.forEach { groupId ->
            val group = archive.readGroup(groupId)
            val componentList = mutableListOf<UIComponent>()
            group.forEach { (fileId, file) ->
                val component = UIComponent(groupId, fileId)
    ...
    private fun ComponentTypeBuilder.readBufferIf3(buf: ByteBuf) {
        ...
        opBase = buf.readString()
        val actionCount = buf.readUnsignedByte().toInt()
        if (actionCount > 0) {
            val actions = Array(actionCount) { buf.readString() }
